...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            """
            # for clip-t loss
            sizes = [(ss[0]*ss[1]).item() for ss in spatial_shapes]
            mask_split = mask_flatten.split(sizes, dim=1)
            memory_split = output_memory.split(sizes, dim=1)
            proposal_split = output_proposals.split(sizes, dim=1)
            proposal_meta = {}
            for li in range(4):
                feat_enc = memory_split[li]
                proposal_enc = proposal_split[li]
                roi_feats = []
                roi_shapes = []
                roi_coords = []
                for bi in range(bs):
                    feat = feat_enc[bi]
                    feat = feat.unsqueeze(0).permute(0, 2, 1).view(1, c, spatial_shapes[li][0], spatial_shapes[li][1])
                    proposals = proposal_enc[bi]
                    valid_proposals = proposals[~mask_split[li][bi]]
                    valid_proposals_resized = valid_proposals.sigmoid() * spatial_shapes[li].repeat(1, 2)
                    roi_feat = torchvision.ops.roi_pool(feat, [valid_proposals_resized], 7)
                    roi_feats.append(roi_feat)
                    roi_shapes.append(spatial_shapes[li])
                    roi_coords.append(valid_proposals_resized)
                proposal_meta["roi_feats_{}".format(li)] = roi_feats
                proposal_meta["roi_shapes_{}".format(li)] = roi_shapes
                proposal_meta["roi_coords_{}".format(li)] = roi_coords
            """
        ...
        # return hs, references, hs_enc, ref_enc, init_box_proposal, proposal_meta
