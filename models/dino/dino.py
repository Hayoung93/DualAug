...
from util.box_ops import box_xyxy_to_cxcywh, box_cxcywh_to_xyxy, box_ltwh_to_xyxy, box_iou
...
class DINO(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, 
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if targets is not None:
            ...
            losses = sum(loss_dict[k] * self.criterion.weight_dict[k] for k in loss_dict.keys() if k in self.criterion.weight_dict)
        else:
            loss_dict = None
            losses = 0

        return loss_dict, losses, out
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_onehot = torch.nn.functional.one_hot(target_classes, num_classes=self.num_classes).to(src_logits.dtype)
        target_classes_onehot[..., 0] = 0
...
def build_dino(args):
    ...
    print('args.num_queries =  {:}'.format(args.num_queries))
    if args.masks:
    ...
    criterion.to(device)
    model.add_attribute("criterion", criterion)
    ...
    return model, criterion, postprocessors, scaler
